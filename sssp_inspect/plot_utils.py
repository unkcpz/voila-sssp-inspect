...
def delta_measure_hist(pseudos: dict, measure_type):
    ...
    for i, (label, output) in enumerate(pseudos.items()):
        ...
        ax.bar(idx+width*i, y_delta, width, color=cmap(label), edgecolor = 'black', linewidth = 1, label=out_label)
        ax.legend()
...
def convergence(pseudos: dict, wf_name, measure_name, ylabel, threshold=None):
    ...
    for i, (label, output) in enumerate(pseudos.items()):
        ...
        try:
            ...
            ax2.plot(x_rho, y_rho, marker='o', color=cmap(label), label=f'cutoff wfc = {wfc_cutoff} Ry')
        except:
            pass
